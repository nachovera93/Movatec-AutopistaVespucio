...
class ActionQuestion(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        Querys(uniqueid)
        ConverterDate()
        dispatcher.utter_message(f'{primernombre}, Estamos llamando de vespucio norte por una boleta pendiente de {monto} pesos vencida el {dia} de {nombreMes} del {anio}. ¿Puede realizar el pago dentro de los próximos 3 días?')
...
class ActionSiPaga(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        dispatcher.utter_message(f"Muchas gracias por su tiempo {primernombre}. Su pago ah quedado agendado para el {dia} de {nombreMes} del {anio}. Para más información puede ingresar a triple doble b .sicc.cl | EXIT")
...
class ActionDonde(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        uniqueid = tracker.sender_id
        #Querys(uniqueid)
        dispatcher.utter_message(f'Nos estamos comunicando desde Vespucio Norte')#{primernombre} podrá pagar dentro de los 3 proximos días?')
        return []
...
class ActionDonde2(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        uniqueid = tracker.sender_id
        Querys(uniqueid)
        dispatcher.utter_message(f'Estamos llamando desde vespucio norte, {primernombre}, podrá pagar dentro de los 3 proximos días?')
        return []
